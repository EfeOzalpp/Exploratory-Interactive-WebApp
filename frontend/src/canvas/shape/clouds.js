import { oscillateSaturation, rgbToHsl, hslToRgb } from '../color/colorUtils.ts';
import { cssToRgbViaP5 } from './utils/colorAdapter.ts';
import { makeArchLobes } from './utils/geom.ts';
import { displacementOsc } from './utils/osc.ts';
import { blendRGB } from './utils/colorBlend.ts';
import { stepAndDrawParticles } from './particles/particle-1.ts';
import { clamp01, val } from './utils/useLerp.ts';
import { applyShapeMods } from './utils/shapeMods.ts';

/* Exposure/contrast helper (channel-space, gentle defaults) */
function applyExposureContrast(rgb, exposure = 1, contrast = 1) {
  const e = Math.max(0.1, Math.min(3, exposure));
  const k = Math.max(0.5, Math.min(2, contrast));
  const adj = (v) => {
    let x = (v / 255) * e;
    x = (x - 0.5) * k + 0.5;
    return Math.max(0, Math.min(1, x)) * 255;
  };
  return { r: Math.round(adj(rgb.r)), g: Math.round(adj(rgb.g)), b: Math.round(adj(rgb.b)) };
}

/* ───────────────── Palettes ───────────────── */
export const CLOUDS_BASE_PALETTE = {
  default: { r: 236, g: 238, b: 242 }, // puff base
  rain:    { r: 20,  g: 165, b: 255 }, // simple blue for rain
};

const CLOUD_BASE = CLOUDS_BASE_PALETTE.default;

/* ───────────────── Defaults (tweakable) ───────────────── */
const RAIN = {
  enabled: true,

  // spawn band (fractions of the rain rect)
  spawnX0: 0.12, spawnX1: 0.88,
  spawnY0: 0.22, spawnY1: 0.0,

  // motion
  angleMin: Math.PI * 0.48,
  angleMax: Math.PI * 0.52,
  speedMin: [280, 160],
  speedMax: [360, 220],
  gravity: 0,
  accelX: 0,
  accelY: 0,

  // jitter
  jitterPos: [3, 0],
  jitterAngle: [0.36, 0],

  // visuals
  count: [24, 18],
  sizeMin: [1, 1.1],
  sizeMax: [1.1, 1.3],
  lengthMin: [2, 8],
  lengthMax: [4, 10],

  // lifetime & fades
  lifeMin: 1,
  lifeMax: 1.6,
  fadeInFrac: 0.15,
  fadeOutFrac: 0.25,

  // per-edge fades (px)
  fadeLeft: 12,
  fadeRight: 12,
  fadeTop: 8,
  fadeBottom: 32,

  // opacity
  alpha: [100, 220],

  // how strongly rain blends with the global gradient
  blend: [0.02, 0.1],
};

// lerp-able cloud tuning (numbers or [min,max])
const CLOUDS = {
  widthEnv:   [0.72, 0.86],   // envelope width vs footprint width
  heightEnv:  [0.24, 0.88],   // envelope height vs cell
  spreadX:    [0.72, 0.82],   // how wide centers spread across the arch
  arcLift:    [0.12, 0.38],   // how high the middle rises (arch)
  rBaseK:     [0.36, 0.46],   // base lobe radius as k * hTop
  rJitter:    [0.08, 0.14],   // lobe radius jitter
  lobeCount:  [6, 9],         // integer after resolve

  sCap:       [0.14, 0.24],   // saturation cap for puff color
  oscAmp:     [0.2, 0.12],    // saturation “breath” amp
  oscSpeed:   [0.32, 0.26],   // saturation “breath” speed

  wobbleAmp:  [1.4, 1.0],     // scales dispAmp/dispAmpY/dispScale
  blend:      [0.4, 0.08],    // ← cloud color blend is now lerp-able
};

const WOBBLE = { ampScale: [0.8, 0.95] };

/* ───────────────── Draw ───────────────── */
export function drawClouds(p, _cx, _cy, _r, opts) {
  const cell = opts?.cell;
  const f = opts?.footprint;
  if (!cell || !f) return;

  const t = ((typeof opts?.timeMs === 'number' ? opts.timeMs : p.millis()) / 1000);
  const seed = (opts?.seed ?? 0) | 0;
  const u = clamp01(opts?.liveAvg ?? 0.5);

  const ex = typeof opts?.exposure === 'number' ? opts.exposure : 1;
  const ct = typeof opts?.contrast === 'number' ? opts.contrast : 1;

  /* ── Layout base ── */
  const x0 = f.c0 * cell;
  const y0 = f.r0 * cell;
  const wTop = f.w * cell;
  const hTop = cell;

  // choose a visual “center” for the cloud arch (used as appear anchor)
  const anchorX = x0 + wTop / 2;
  const anchorY = y0 + hTop * 0.60;

  /* ── Resolve cloud geometry (lerp-able) ── */
  const wEnv = wTop * val(CLOUDS.widthEnv,  u);
  const hEnv = hTop * val(CLOUDS.heightEnv, u);
  const spreadX = val(CLOUDS.spreadX, u);
  const arcLift = val(CLOUDS.arcLift, u);
  const rBase   = hTop * val(CLOUDS.rBaseK, u);
  const rJitter = val(CLOUDS.rJitter, u);
  const lobeCount = Math.max(3, Math.round(val(CLOUDS.lobeCount, u)));

  const lobes = makeArchLobes(
    anchorX,
    anchorY,
    wEnv,
    hEnv,
    { count: lobeCount, spreadX, arcLift, rBase, rJitter, seed }
  ) || [];

  /* ── Cloud color (cap + “breath”, both lerp-able) ── */
  const cloudBlendDefault = val(CLOUDS.blend, u); // lerp-able default
  const cloudBlend = typeof opts?.cloudBlend === 'number' ? opts.cloudBlend : cloudBlendDefault;

  const baseTint =
    (typeof opts?.cloudCss === 'string' && opts.cloudCss.trim().length > 0)
      ? cssToRgbViaP5(p, opts.cloudCss)
      : blendRGB(CLOUD_BASE, opts?.gradientRGB, cloudBlend);

  const sMax = Math.max(0, Math.min(1, val(CLOUDS.sCap, u)));
  const { h, s, l } = rgbToHsl(baseTint);
  const capped = hslToRgb({ h, s: Math.min(s, sMax), l });

  let cloudRgb = oscillateSaturation(capped, t, {
    amp:   (typeof opts?.oscAmp === 'number' ? opts.oscAmp : val(CLOUDS.oscAmp, u)),
    speed: (typeof opts?.oscSpeed === 'number' ? opts.oscSpeed : val(CLOUDS.oscSpeed, u)),
    phase: opts?.oscPhase ?? 0,
  });

  cloudRgb = applyExposureContrast(cloudRgb, ex, ct);

  /* ── Wobble (lerp-able) ── */
  const wobbleK = val(CLOUDS.wobbleAmp, u) * val(WOBBLE.ampScale, u);
  const ampX = (opts?.dispAmp ?? Math.min(12, Math.max(6, Math.round(hTop * 0.12)))) * wobbleK;
  const ampY = ((typeof opts?.dispAmpY === 'number') ? opts.dispAmpY : Math.round(ampX * 0.85)) * wobbleK;
  const ampS = (Math.max(0, Math.min(0.25, opts?.dispScale ?? 0.12))) * wobbleK;
  const fX = Math.max(0.01, opts?.dispSpeed ?? 0.22);
  const fY = fX * 0.85;
  const fS = fX * 0.60;

  /* ── APPEAR/SCALE envelope for the cloud *shape* (NOT the emitter) ── */
  const appear = applyShapeMods({
    p,
    x: anchorX,
    y: anchorY,
    r: Math.min(wTop, hTop),
    opts: {
      alpha: Number.isFinite(opts?.cloudAlpha) ? opts.cloudAlpha : 235,
      timeMs: opts?.timeMs,
      liveAvg: opts?.liveAvg,
      rootAppearK: opts?.rootAppearK, // if the reconciler feeds one
    },
    mods: {
      appear: {
        scaleFrom: 0.0,
        alphaFrom: 0.0,
        anchor: 'center',
        ease: 'back',
        backOvershoot: 1.2,
      },
      // wobble handled manually per lobe; keep sizeOsc off here
      sizeOsc: { mode: 'none' },
    }
  });

  const cloudAlpha = (typeof appear.alpha === 'number') ? appear.alpha : (Number.isFinite(opts?.cloudAlpha) ? opts.cloudAlpha : 235);

  /* ── RAIN FIRST (under clouds). Keep emitter in world-space (unscaled). ── */
  if (RAIN.enabled) {
    const rect = { x: x0, y: y0 + hTop * 0.5, w: wTop, h: hTop * 2.5 };

    const speedMin    = val(RAIN.speedMin, u);
    const speedMax    = val(RAIN.speedMax, u);
    const jitterPos   = val(RAIN.jitterPos, u);
    const jitterAngle = val(RAIN.jitterAngle, u);
    const count       = Math.max(8, Math.floor(val(RAIN.count, u)));
    const lengthMin   = val(RAIN.lengthMin, u);
    const lengthMax   = val(RAIN.lengthMax, u);
    const baseAlpha   = Math.round(val(RAIN.alpha, u));

    // sync fade with appear alpha, but keep rectangle unscaled
    const syncedAlpha = Math.round(baseAlpha * (cloudAlpha / 255));

    // NEW: size min/max lerp
    const sizeMin     = val(RAIN.sizeMin, u);
    const sizeMax     = Math.max(sizeMin, val(RAIN.sizeMax, u));

    // Rain blend: default lerps (inverted) if no explicit opts.rainBlend
    const rainBlend =
      typeof opts?.rainBlend === 'number'
        ? opts.rainBlend
        : val(RAIN.blend, 1 - u); // as avg→0, blend→higher

    let rainTint =
      (typeof opts?.rainCss === 'string' && opts.rainCss.trim().length > 0)
        ? cssToRgbViaP5(p, opts.rainCss)
        : blendRGB(CLOUDS_BASE_PALETTE.rain, opts?.gradientRGB, rainBlend);

    rainTint = applyExposureContrast(rainTint, ex, ct);

    const rainColor = { r: rainTint.r, g: rainTint.g, b: rainTint.b, a: syncedAlpha };
    const dt = Math.max(0.001, (p.deltaTime || 16) / 1000);

    stepAndDrawParticles(p, {
      key: `${f.r0}:${f.c0}:${f.w}x${f.h}:${seed}:rain`,
      rect,
      mode: 'line',
      color: rainColor,

      spawn: { x0: RAIN.spawnX0, x1: RAIN.spawnX1, y0: RAIN.spawnY0, y1: RAIN.spawnY1 },
      angle: { min: RAIN.angleMin, max: RAIN.angleMax },

      speed: { min: speedMin, max: speedMax },
      gravity: RAIN.gravity,
      accel: { x: RAIN.accelX, y: RAIN.accelY },

      jitter: { pos: jitterPos, velAngle: jitterAngle },

      count,
      size: { min: sizeMin, max: sizeMax },
      length: { min: lengthMin, max: lengthMax },
      sizeHz: 8,
      lenHz: 6,

      lifetime: { min: RAIN.lifeMin, max: RAIN.lifeMax },
      fadeInFrac: RAIN.fadeInFrac,
      fadeOutFrac: RAIN.fadeOutFrac,

      edgeFadePx: { left: RAIN.fadeLeft, right: RAIN.fadeRight, top: RAIN.fadeTop, bottom: RAIN.fadeBottom },
      respawn: true,
    }, dt);
  }

  /* ── CLOUDS SECOND (draw above rain). Apply appear transform only to lobes. ── */
  p.push();
  p.translate(appear.x, appear.y);
  p.scale(appear.scaleX, appear.scaleY);
  p.translate(-anchorX, -anchorY);

  p.noStroke();
  p.fill(cloudRgb.r, cloudRgb.g, cloudRgb.b, cloudAlpha);

  for (const l of lobes) {
    const { dx, dy, sc } = displacementOsc(t, l.i, {
      ampX, ampY, ampScale: ampS, freqX: fX, freqY: fY, freqScale: fS, seed
    });
    // draw in local space (after transform), so subtract anchor center:
    const lx = l.x; // already centered on anchorX in makeArchLobes
    const ly = l.y; // already centered on anchorY
    p.circle(lx + dx, ly + dy, l.r * sc * 2);
  }

  p.pop();
}
